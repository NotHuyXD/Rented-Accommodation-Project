...
import {
  ...
  CreditCard, Receipt, Search, XCircle, Trash2, Calendar
} from 'lucide-react';
...
export default function TenantDashboard() {
  ...
  const {
    contracts, invoices, rentalRequests, bookmarks, appointments,
    fetchContracts, fetchInvoices, fetchRentalRequests, fetchBookmarks, fetchAppointments, cancelAppointment
  } = useAppStore();
  ...
  const handleCancelAppointment = async (id: string) => {
    if (!(await confirmAsync('Xác nhận', 'Bạn có chắc muốn hủy lịch xem phòng này?'))) return;
    setCancelling(id);
    try {
      const ok = await cancelAppointment(id);
      if (ok) alertQuick('success', 'Đã hủy lịch xem phòng');
      else alertQuick('error', 'Lỗi hủy lịch hẹn');
    } finally {
      setCancelling(null);
    }
  };
        ...
        <div className="dashboard-grid">
          ...
          <div className="dashboard-card dashboard-card-full" style={{ marginTop: '24px' }}>
            ...
            {appointments.length > 0 ? (
                  ...
                  return (
                    <div key={app.id} className="dashboard-ticket-item">
                      ...
                      <div className="dashboard-ticket-info">
                        ...
                        <p>
                          <span>Hẹn xem: <strong>{formatDate(app.appointment_date)}</strong> lúc <strong>{app.appointment_time}</strong></span>
                          {app.landlord_name && <span> · Chủ trọ: <strong>{app.landlord_name}</strong></span>}
                          {app.landlord_phone && <span> · SĐT: <strong>{app.landlord_phone}</strong></span>}
                        </p>
                        {app.message && (
                      ...
                      <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexShrink: 0 }}>
                        ...
                        {(isPending || app.status === 'confirmed') && (
                          <button
                            className="btn btn-sm"
                            style={{ 
                              background: '#ef444415', color: '#ef4444', border: 'none',
                          ...
                          >
                            <XCircle size={14} />
                            {cancelling === app.id ? 'Đang xử lý...' : 'Hủy lịch'}
                          </button>
                        )}
                      </div>
                    </div>
              ...
              <p style={{ color: 'var(--text-tertiary)', padding: '24px' }}>Chưa đặt lịch xem phòng nào</p>
            )}
          </div>
        </div>
      </div>
