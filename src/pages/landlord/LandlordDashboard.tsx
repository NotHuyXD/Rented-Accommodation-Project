...
import {
  ...
  Wrench, MessageCircle, X, Check, XCircle, Calendar
} from 'lucide-react';
...
export default function LandlordDashboard() {
  ...
  const { rentalRequests, fetchRentalRequests, contracts, fetchContracts, appointments, fetchAppointments, confirmAppointment, cancelAppointment } = useAppStore();
  ...
  const handleConfirmAppointment = async (id: string) => {
    if (!(await confirmAsync('Xác nhận', 'Bạn có chắc muốn xác nhận lịch xem phòng này?'))) return;
    setProcessing(true);
    try {
      const ok = await confirmAppointment(id);
      if (ok) alertQuick('success', 'Đã xác nhận lịch xem phòng');
      else alertQuick('error', 'Lỗi xác nhận lịch hẹn');
    } finally {
      setProcessing(false);
    }
  };
  ...
  const handleCancelAppointment = async (id: string) => {
    if (!(await confirmAsync('Xác nhận', 'Bạn có chắc muốn từ chối/hủy lịch xem phòng này?'))) return;
    setProcessing(true);
    try {
      const ok = await cancelAppointment(id);
      if (ok) alertQuick('success', 'Đã từ chối/hủy lịch hẹn');
      else alertQuick('error', 'Lỗi xử lý lịch hẹn');
    } finally {
      setProcessing(false);
    }
  };
    ...
    <div className="dashboard-page">
      <div className="container">
        ...
        <div className="dashboard-grid">
          ...
          <div className="dashboard-card dashboard-card-full" style={{ marginTop: '24px' }}>
            <div className="dashboard-card-header">
              <h2>Lịch hẹn xem phòng ({pendingAppointments.length} chờ duyệt)</h2>
            </div>
            <div className="dashboard-tickets">
              {appointments.length > 0 ? (
                appointments.slice(0, 10).map(app => {
                  const isPending = app.status === 'pending';
                  let statusBg = '#10b98115'; let statusColor = '#10b981'; let statusText = 'Đã xác nhận';
                  if (app.status === 'pending') { statusBg = '#f59e0b15'; statusColor = '#f59e0b'; statusText = 'Chờ duyệt'; }
                  else if (app.status === 'cancelled') { statusBg = '#ef444415'; statusColor = '#ef4444'; statusText = 'Đã hủy'; }
                  else if (app.status === 'completed') { statusBg = '#3b82f615'; statusColor = '#3b82f6'; statusText = 'Hoàn thành'; }

                  return (
                    <div key={app.id} className="dashboard-ticket-item">
                      <div className="dashboard-ticket-icon" style={{ background: statusBg, color: statusColor }}>
                        {isPending ? <Clock size={18} /> : <Calendar size={18} />}
                      </div>
                      <div className="dashboard-ticket-info">
                        <h4>{app.room_title || 'Phòng trọ'}</h4>
                        <p>
                          <strong>{app.tenant_name || 'Khách thuê'}</strong> - Hẹn xem: <strong>{formatDate(app.appointment_date)}</strong> lúc <strong>{app.appointment_time}</strong>
                          {app.tenant_phone && <span> · SĐT: <strong>{app.tenant_phone}</strong></span>}
                        </p>
                        {app.message && (
